...
categorical = ["eventType","opChapNum","op02ChapNum","op03ChapNum","ethnicity3Groups","Acuity"]
postevent = ["LOS","InHospitalMortality","Mortality30","Mortality365","Mortality730"]
#ordinals and binaries are left alone
ordinals = ["Dep_score","opSeverity","ASA"]
needsimputation = ["opSeverity","ASA","IsEmergency","Dep_score"]


m = 40  # This is the number of imputed datasets created, using 50 when running for real
epochs = 500  # This should be set to 500 when it is run for real.
doVerbose = True  # This should be set to False for the real run.
debug = False
layers = [64, 64]
doTrain = True
doTargetTrain = True
savePath = "C:\\PDH\\MIDAS\\tmp\\"
datasetName = "MOHMidasE500m40VAE"
...
df = pd.read_csv('c:\pdh\moh\MOHFinalPreImputeTrainV2.csv')
print(df.head())
print(df.shape)
trainlength = len(df)
...
dft = pd.read_csv('c:\pdh\moh\MOHFinalOrgTestV2.csv')
print(dft.head())
print(dft.shape)
testlength = len(dft)
...
imputer = Midas(layer_structure=layers, train_batch=64, vae_layer=True, latent_space_size = 4, vae_alpha = 1.0 , seed=42, savepath=fullSavePath)
...
def outputdatasets(columns_list1, data_additional1, imputer1, orgdf1, fullSavePath1, suffix=''):
    ...
    for dataset in imputer1.output_list:
        ...
        decimals = pd.Series([0, 0, 0, 0], index=['ASA', 'IsEmergency', 'OpSeverity', 'Dep_score'])
...
outputdatasets(da_columns_list, dataadditional, imputer, orgdf, fullSavePath)
...
outputdatasets(da_columns_list, datatestadditionalorg, imputer, orgdft, fullSavePath, suffix='TEST')
