...
---
Objective: Compare impact of MI on In-Hospital Mortality prediction with MOH data

1. MOHInitialClean.Rmd
2. MOHCreateDatasets.R - creates test and train datasets plus mice imputation
3. MOHFitAndPredict.Rmd (This file)
4. HelperFunctions1.R

TBD:MOHExploratory.Rmd (some plots wrt missingness)
...
m_30dayformula = Mortality30 ~ 1 + Acuity + Age + HasCancer + threeGroupASA + Surg_grade_4_5 + surg01Spec + gender + ethnicity3Groups

m_365dayformula = Mortality365 ~ 1 + Acuity + Age + HasCancer + threeGroupASA + surg01Spec365 + gender + ethnicity3Groups

m_730dayformula = Mortality730 ~ 1 + Acuity + Age + HasCancer + threeGroupASA + surg01Spec365 + gender + ethnicity3Groups

#most of the prepwork have already been incorporated into our helper classes so minimal transformations is required
prepNZRisk <- function(df) {
  df$threeGroupASA = ifelse(df$ASA == 1, yes = "ASA1/2"
                                  , no = ifelse(df$ASA == 2, yes = "ASA1/2"
                                      , no = ifelse(df$ASA == 3, yes = "ASA 3"
                                          , no = ifelse(df$ASA == 4, yes = "ASA 4/5"
                                              , no = "ASA 4/5"))))
  
  df$threeGroupASA = relevel(as.factor(df$threeGroupASA), ref = "ASA1/2")
  
  df$gender = ifelse(df$IsMale == 1, yes = "M", no = "F")
  df$gender = relevel(as.factor(df$gender), ref = "F")
  ...
  # Depending on the time frame (30d vs longer) digestive is included or not included or not included. It was statistically significant at 30d but not any longer timeframe
  # Two year model - remove surgery grade + digestive system (same op type classes as 365days)
  op_types365 = c("1_nervousSystem","other","other","other","other","7_respiratorySystem","8_vascularSystem"
                 ,"other","other","other","other","other","other","other","other")
  
  df$surg01Spec365 = mapvalues(x=df$opChapNum, from = levels(as.factor(as.numeric(as.character(df$opChapNum)))), to=op_types365)
  df$surg01Spec365 = relevel(as.factor(df$surg01Spec365), ref = "other")
  return(df)
}
...
dfCCtrain <- read.csv(paste0(parentfolder,"MOHFinalCompleteCaseTrain.csv"), header=TRUE
...
midsMICEtrain = readRDS(paste0(parentfolder,"MiceMohFinalTrain.rds"))
...
dfMRtrain = readRDS(paste0(parentfolder,"MohFinalMRTrainV2.rds"))
trainOrg = read.csv(paste0(parentfolder,"MOHFinalPreImputeTrainV2.csv"), header=TRUE
                      ,comment.char = "",  skip =0,  check.names = FALSE, na.strings=c("NA","NaN", ""),colClasses=traincolInfo)
...
#str(dfMRtrain)
#str(dfMRtrain2)
#dfmr0 = dfMRtrain[dfMRtrain$.imp==0,]
#dfmr1 = dfMRtrain[dfMRtrain$.imp==1,]


dfMIDAStrain = read.csv(paste0(parentfolder,"MOHMidasE250m40_full.csv"))
#dfmidas0 = dfMIDAStrain[dfMIDAStrain$.imp==0,]
#dfmidas1 = dfMIDAStrain[dfMIDAStrain$.imp==1,]

dfMIDASVAEtrain = read.csv(paste0(parentfolder,"MOHMidasE250m40VAE_full.csv"))
...
dfCCtrainPrepped=prepNamesAndLevel(dfCCtrainPrepped)
dfCCtrainPrepped=prepNZRisk(dfCCtrainPrepped)

dfMICEtrainPrepped=prepDataSet(dfMICEtrain)
dfMICEtrainPrepped=prepNamesAndLevel(dfMICEtrainPrepped)
dfMICEtrainPrepped=prepNZRisk(dfMICEtrainPrepped)

dfMRtrainPrepped=prepDataSet(dfMRtrain2)
dfMRtrainPrepped=prepFactors(dfMRtrainPrepped)
dfMRtrainPrepped=prepNZRisk(dfMRtrainPrepped)

dfMIDAStrainPrepped=prepDataSet(dfMIDAStrain)
dfMIDAStrainPrepped=prepFactors(dfMIDAStrainPrepped)
dfMIDAStrainPrepped=prepNZRisk(dfMIDAStrainPrepped)

dfMIDASVAEtrainPrepped=prepDataSet(dfMIDASVAEtrain)
dfMIDASVAEtrainPrepped=prepFactors(dfMIDASVAEtrainPrepped)
dfMIDASVAEtrainPrepped=prepNZRisk(dfMIDASVAEtrainPrepped)
...
fitMICEmids <- with(midsMICEtrainPrepped,  glm(formula = Mortality30 ~ 1 + Acuity + Age + HasCancer + threeGroupASA + Surg_grade_4_5 + surg01Spec + gender + ethnicity3Groups, family = binomial("logit"), maxit=100), print=F)
...
fitMICE <- lapply(1:m, function(x) glm(formula=m_30dayformula, family=binomial("logit"), maxit=100, data=dfMICEtrainPrepped[dfMICEtrainPrepped$.imp==x,]))

#fit the model for missRanger imputed dataset
fitMR <- lapply(1:m, function(x) glm(formula=m_30dayformula, family=binomial("logit"), maxit=100, data=dfMRtrainPrepped[dfMRtrainPrepped$.imp==x,]))

#fit the model for MIDAS imputed dataset
fitMIDAS <- lapply(1:m, function(x) glm(formula=m_30dayformula, family=binomial("logit"), maxit=100, data=dfMIDAStrainPrepped[dfMIDAStrainPrepped$.imp==x,]))

#fit the model for MIDAS imputed dataset
fitMIDASVAE <- lapply(1:m, function(x) glm(formula=m_30dayformula, family=binomial("logit"), maxit=100, data=dfMIDASVAEtrainPrepped[dfMIDASVAEtrainPrepped$.imp==x,]))

#fit the model for complete case dataset
fitCC <- glm(formula = m_30dayformula, family = binomial("logit"), maxit=100, data=dfCCtrainPrepped)
```
...
cbind("Complete Case"=round(fitCCstderr,2), 
      MICE=round(sqrt(fitMICETotalVariance)-fitCCstderr,2),
      missRanger=round(sqrt(fitMRTotalVariance)-fitCCstderr,2),
...
cat('Fraction of Missing Information for imputed models vs proportion of missing information (complete case)\n')
...
cbind(MICE=round(MICEfmi,2), missRanger=round(MRfmi,2), 
      MIDAS=round(MIDASfmi,2), MIDASVAE=round(MIDASVAEfmi,2)
      , "Prop. Of Missing Info"=round(propOfMissingInfo,2))
...
testcolInfo = c("factor", "factor", "factor", "factor", "factor", "factor", "integer", "factor", "factor", "factor"
            ,"factor","factor", "factor", "factor", "integer", "integer", "integer", "integer", "integer", "integer"
            , "integer", "integer", "factor","factor", "factor", "integer", "factor")
dfCCtest <- read.csv(paste0(parentfolder,"MOHFinalCompleteCaseTest.csv"), header=TRUE
                ,comment.char = "",  skip =0,  check.names = FALSE, na.strings=c("NA","NaN", ""),colClasses=traincolInfo)

#Load MICE imputed train dataset - previously saved as a mids object
midsMICEtest = readRDS(paste0(parentfolder,"MiceMohFinalTest.rds"))
dfMICEtest =  complete(midsMICEtest, "long", include=TRUE)
# plot(midsMICEtest)

#,colClasses=testcolInfo
#MR 0-39 are imputed sets, lets fix this so it has similar structure as mice and midas
dfMRtest = readRDS(paste0(parentfolder,"MohFinalMRTestV2.rds"))
testOrgStripped = read.csv(paste0(parentfolder,"MOHFinalPreImputeTestV2.csv"), header=TRUE
...
dfMIDAStest = read.csv(paste0(parentfolder,"MOHMidasE250m40TEST_full.csv"))

dfMIDASVAEtest = read.csv(paste0(parentfolder,"MOHMidasE250m40VAETEST_full.csv"))

#prep the datasets so they can be used for fitting and predictions (releveling factors, mapping to higher grain, changing datatypes, etc.)
dfCCtestPrepped=prepDataSet(dfCCtest)
dfCCtestPrepped=prepNamesAndLevel(dfCCtestPrepped)
dfCCtestPrepped=prepNZRisk(dfCCtestPrepped)

dfMICEtestPrepped=prepDataSet(dfMICEtest)
dfMICEtestPrepped=prepNamesAndLevel(dfMICEtestPrepped)
dfMICEtestPrepped=prepNZRisk(dfMICEtestPrepped)

dfMRtestPrepped=prepDataSet(dfMRtest2)
dfMRtestPrepped=prepFactors(dfMRtestPrepped)
dfMRtestPrepped=prepNZRisk(dfMRtestPrepped)

dfMIDAStestPrepped=prepDataSet(dfMIDAStest)
dfMIDAStestPrepped=prepFactors(dfMIDAStestPrepped)
dfMIDAStestPrepped=prepNZRisk(dfMIDAStestPrepped)

dfMIDASVAEtestPrepped=prepDataSet(dfMIDASVAEtest)
dfMIDASVAEtestPrepped=prepFactors(dfMIDASVAEtestPrepped)
dfMIDASVAEtestPrepped=prepNZRisk(dfMIDASVAEtestPrepped)
#visually verify structure
# str(dfCCtrainPrepped)
#str(dfMICEtrainPrepped)
#str(dfMRtrainPrepped)
...
dfFulltestPrepped=prepNZRisk(dfFulltestPrepped)
...
MICEmodelmatrix <- lapply(1:m, function(x)  model.matrix( ~ 1 + Acuity + Age + HasCancer + threeGroupASA + Surg_grade_4_5 + surg01Spec + gender + ethnicity3Groups
                                                , data=dfMICEtestPrepped[dfMICEtestPrepped$.imp==x,]))

MRmodelmatrix <- lapply(1:m, function(x)  model.matrix(  ~ 1 + Acuity + Age + HasCancer + threeGroupASA + Surg_grade_4_5 + surg01Spec + gender + ethnicity3Groups
                                                , data=dfMRtestPrepped[dfMRtestPrepped$.imp==x,]))

MIDASmodelmatrix <- lapply(1:m, function(x)  model.matrix(  ~ 1 + Acuity + Age + HasCancer + threeGroupASA + Surg_grade_4_5 + surg01Spec + gender + ethnicity3Groups
                                                , data=dfMIDAStestPrepped[dfMIDAStestPrepped$.imp==x,]))

MIDASVAEmodelmatrix <- lapply(1:m, function(x)  model.matrix(  ~ 1 + Acuity + Age + HasCancer + threeGroupASA + Surg_grade_4_5 + surg01Spec + gender + ethnicity3Groups
...
cat("Mean Loss\n")
cbind(MICE = round(MICEmeanloss,3), missRanger = round(MRmeanloss,3), MIDAS = round(MIDASmeanloss,3), MIDASVAE = round(MIDASVAEmeanloss,3), "Complete Case"= round(CCmeanloss,3), "Adjusted CC" = round(adjustedCCmeanloss,3))

cat("\nSum Loss\n")
cbind(MICE = round(MICEsumloss,2), missRanger = round(MRsumloss,2), MIDAS = round(MIDASsumloss,2), MIDASVAE = round(MIDASVAEsumloss,2), "Complete Case"= round(CCsumloss,2), "Adjusted CC" = round(adjustedCCsumloss,2))
...
# plot_HL <- function(hldata1, hllmodel1, hldata2, hllmodel2, title = "Calibration plot of NMDS mortality model", xlim = 1.0,ylim = 1.0, showabline = TRUE) {
#   abltype = 8
#   if (showabline==FALSE) {
#       abltype = 0
#   } 
...
#     
#     geom_point(data=hldata1, aes(y=meanobs, x=meanpred), col = "blue", shape=1, size=1.5, stroke=1.5) + # Plotting the bubbles. PLot on observed and pred
#     geom_abline(slope=hllmodel1[2], intercept = hllmodel1[1], col="blue", lwd=0.4, lty=abltype) + # Adding in the blue line which shows the best fit line through our model
#     
#     geom_point(data=hldata2, aes(y=meanobs, x=meanpred), col = "red", shape=1, size=1.5, stroke=1.5) + # Plotting the bubbles. PLot on observed and pred
#     geom_abline(slope=hllmodel2[2], intercept = hllmodel2[1], col="red", lwd=0.4, lty=abltype) + # Adding in the red line which shows the best fit line through our model
#     
...
plot_HL(MIDAShl, MIDAShl.lm, CChl, CChl.lm, "MIDAS", "Complete Case", xlim=0.1, ylim=0.1, showabline=FALSE)
```
